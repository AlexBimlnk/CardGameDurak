...
/// <summary xml:lang = "ru">
/// Стратегия, используемая ботом на дополнительной сложности.
/// </summary>
public class AdditionalStrategy: IBotStrategy
{
    /// <inheritdoc/>
        ...
        if (deckSize > 11)
            ...
            resultCard ??= simpleCards.Where(x => desktopCards.Any(y => x.Rank == y.Rank))
                        ...
                        ?? handCards.Where(x => desktopCards.Any(y => x.Rank == y.Rank))
                                    .Where(x => x.Rank < 10)
                                    .MinBy(x => x.Rank);
        else resultCard ??= simpleCards.Where(x => desktopCards.Any(y => x.Rank == y.Rank))
                                       .MinBy(x => x.Rank)
                        ?? handCards.Where(x => desktopCards.Any(y => x.Rank == y.Rank))
                                    .MinBy(x => x.Rank);
        ...
        out ICard closedCard)
    {
        ...
        var simpleCards = handCards.Where(x => !x.IsTrump);
        var trumpCards = handCards.Where(x => x.IsTrump);
        resultCard = simpleCards.Where(x => needClosed.Rank < x.Rank)
